...
document must be produced and positively marked ready earlier in the same build; this plugin does not start an
application, infer freshness from an old target file, or implement springdoc scanning. Bind the execution only in
the service's generation-owner module so plugin inheritance cannot create duplicate writers.
